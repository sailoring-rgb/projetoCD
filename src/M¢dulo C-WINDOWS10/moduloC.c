...
//Determina qual é o maior numero de um array. Utilizado para determinar o maior bloco.
...
//Função para tornar um array num inteiro;
...
//Função para calcular os tamanhos dos blocos;
//Recebe apontador para o array de caracteres do ficheiro ".cod" + número de blocos + apontador para array dos tamanhos.
void tamanho_blocos(char *arrayCod, int n_blocos, int *tamanhos){

    int cont_posicao = 4;                                                               //Contador para percorrer o array. Ignora as primeiras 4 casas pois não interessam.
    int contador_at = 0;                                                                //Contador de @
    int index_temp = 0;                                                                 //Index do array temporario.
    int index_t_blocos = 0;                                                             //Posiçao no array onde vai ser guardado o tamanho do bloco.
    ...
    while(contador_at < 2*n_blocos){                                                    //O tamanho do bloco encontra-se entre 2 @. Enquanto o contador de @ não for >= 2*número de blocos
        if(arrayCod[cont_posicao] == '@'){                                                 //continua a procurar o tamanho do proximo bloco tamanho do proximo bloco.
            contador_at++;
            if(contador_at%2 == 0){                                                     //enquanto o contador de @ for par apenas passa posiçao do arary a frente.
                cont_posicao++;
...
//Função para armazenar apenas o conteudo do bloco em array.
//Recebe apontador para o array de caracteres do ficheiro ".cod" + o index onde queremos começar + apontador para o array de caracteres do bloco
int array_bloco (char *arrayCod, int index_Cod, char *bloco_Cod){

    int index_bloco = 0;

    while(arrayCod[index_Cod] != '@'){                  //enquanto não for @ vai guardar o valor no arary bloco_cod
...
//Função para substituir uma Matriz
...
//Função para calcular quantos zeros serão preciso adicionar ao *str de modo ao str_cnt ser divisivel por 8;
...
// Função para imprimir o codigo codificado no ficheiro.
...
void imprime_no_ficheiro (unsigned char *str , int str_cnt, FILE *fpShaf){
    ...
    fprintf(fpShaf,"%c%d%c",'@',str_cnt/8,'@');             //imprime o tamanho do bloco depois da compressão;
    ...
    for(int i = 0; i < str_cnt/8; i++){
        ...
        decimal = (int) strtol(oito_char, NULL, 2);         //Os caracteres que constituem o oito_char são '0' ou '1' formando número binario;
        fputc(decimal, fpShaf);                              //O strtol converte esse número binario em decimal;
                                                            //fputc imprime no ficheiro o char correspondente ao decimal;
    }
}


//Função para criar uma string a partir de uma matriz de caracteres
int matriz_to_string(char **matriz, int t_bloco_atual, FILE *fpShaf){
    ...
    for(int i = 0; i < t_bloco_atual; i++){
        ...
        int realloc_size = (int)strlen(matriz[i]);                                  //Quantidade de memoria que vamos precisar de alocar para o array na posiçao i da matriz;
        memory_size = memory_size + realloc_size;                                   //Para um realloc precisamos da memoria total que queremos alocar, então somamos a nova memoria à que ja tinhamo;
        str = ( unsigned char*)realloc(str,  memory_size * sizeof( unsigned char));
        for(int x=0; x<strlen((matriz[i])); x++) {                                  //O ciclo for repete tantas vezes quanto o tamanho do array que esta na posiçao i da matriz;
            str[str_cnt] = matriz[i][x];                                            //Copiar o array que esta na posiçao i para o str;
            str_cnt++;
        }
    }

    int numero_zeros = quantos_zeros(str,str_cnt);                                  //função quantos_zeros;
    ...
    str[str_cnt] = '\0';
    imprime_no_ficheiro(str, str_cnt, fpShaf);                                      //funçao imprime_no_ficheiro;
    free(str);
    return str_cnt;
}


//Funçao para fazer a conversão
int conversao(char *bloco_Cod, char *bloco_Ori, int t_bloco_atual, FILE *fpShaf, int maior_bloco){
    ...
    while(bloco_Cod[index_Cod] != NULL){                        //vai percorrer o *bloco_Cod ignorando tudo que seja ';'
...
//Função vai escrever os blocos dos ficheiros nos *bloco_Cod e *bloco_Ori
...
//Imprime no final do ficheiro taxa de compressão de cada bloco.
...
//Função para calcular a taxa total de compressao do ficheiro.
...
//Funçao principal onde se inicializa o temporizador, lê e criam ficheiros e se imprimem as informações finais.
void fPrincipal(char *nomeFicheiroOriginal)
{
    clock_t Ticks[2]; //numero de ticks de relogio, 0 para começar e 1 para acabar.
    int n_blocos = 0;
    int sizeOri = 0;
    int sizeCod = 0;
    int maior_bloco = 0;
...
//FINALIZAR TEMPORIZADOR
    Ticks[1] = clock();
    double Tempo = (Ticks[1] - Ticks[0]) * 1000.0 / CLOCKS_PER_SEC; //calcula o tempo de execução em milissegundos.
